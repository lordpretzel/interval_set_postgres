 CREATE OR REPLACE FUNCTION range_set_multiply(l int[][], r int[][])
 RETURNS int[][] AS $$
 DECLARE
    result int[][];
    i int;
    ...
    FOR i IN array_lower(l, 1)..array_upper(l, 1) LOOP
        FOR j IN array_lower(r, 1)..array_upper(r, 1) LOOP
            ...
            result := result || ARRAY[ARRAY[
                    LEAST(l[i][1] * r[j][1], l[i][1] * r[j][2], l[i][2] * r[j][1], l[i][2] * r[j][2]),
                    GREATEST(l[i][1] * r[j][1], l[i][1] * r[j][2], l[i][2] * r[j][1], l[i][2] * r[j][2])
                ]];
        END LOOP;
    END LOOP;
